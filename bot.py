...
async def song(client, message):
    ...
    if args.startswith(" "):
        await message.reply("WHICH SONG DO YOU NEEDED?")
        return ""
    pak = await message.reply('DOWNLOADING...UNDER....PROCESS...PLZ...WAIT...')
    try:
        # @TG <BOTS>
...
async def deezer(_, message):
    if len(message.command) < 2:
        await message.reply_text("WHICH SONG DO YOU NEEDED")
        return
...
async def play_track(client, message):
    ...
    if VOICE_CHATS and message.chat.id in VOICE_CHATS:
        text = f'▶️ Playing **{audio.title}** here by ME...'
    else:
        try:
            group_call = GroupCall(client, input_filename)
            await group_call.start(message.chat.id)
        except RuntimeError:
            await message.reply('VC WAS NOT YET STARTED')
            return
        VOICE_CHATS[message.chat.id] = group_call
    await a.edit(f'▶️ Playing **{audio.title}** here by ME...')
